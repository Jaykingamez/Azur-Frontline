...
from objects import AzurObject, get_bare_hands
...
class EquipmentHandler:
    ...
    def max_slots(self):
        ...
        """
        return getattr(self.obj, Ability.BOD.value, 20) + (getattr(self.obj, Ability.BOD.value, 20) / 2)
    ...
    def validate_slot_usage(self, obj):
        ...
        Args:
            obj (AzurObject): The object to add.

        """
        if not inherits_from(obj, AzurObject):
    ...
    def get_current_slot(self, obj):
        ...
        Args:
            obj (AzurObject): The object to check.
    ...
    def armor(self):
        ...
        return sum(
            (
                ...
                getattr(slots[WieldLocation.UPPER_BODY], "armor", 0),
                getattr(slots[WieldLocation.LOWER_BODY], "armor", 0),
    ...
    def weapon(self):
        ...
        if not weapon:
            weapon = slots[WieldLocation.LEFT_HAND]
        if not weapon:
    ...
    def display_loadout(self):
        ...
        if two_hands:
            ...
            one_hands = slots[WieldLocation.LEFT_HAND]
            if one_hands:
                weapon_str = f"You are wielding {one_hands} in one hand."
            shield = slots[WieldLocation.RIGHT_HAND]
            if shield:
                shield_str = f"You have {shield} in your off hand."

        armor = slots[WieldLocation.UPPER_BODY] + slots[WieldLocation.LOWER_BODY]
        if armor:
    ...
    def move(self, obj):
        ...
        if use_slot is WieldLocation.TWO_HANDS:
            ...
            to_backpack = [slots[WieldLocation.LEFT_HAND], slots[WieldLocation.RIGHT_HAND]]
            slots[WieldLocation.LEFT_HAND] = slots[WieldLocation.RIGHT_HAND] = None
            slots[use_slot] = obj
        elif use_slot in (WieldLocation.LEFT_HAND, WieldLocation.RIGHT_HAND):
    ...
    def remove(self, obj_or_slot):
        ...
        Args:
            obj_or_slot (AzurObject or WieldLocation): The specific object or
    ...
    def get_wieldable_objects_from_backpack(self):
        ...
        return [
            ...
            in (WieldLocation.LEFT_HAND, WieldLocation.TWO_HANDS, WieldLocation.RIGHT_HAND)
        ]
    ...
    def get_wearable_objects_from_backpack(self):
        ...
        return [
            obj
            for obj in self.slots[WieldLocation.BACKPACK]
            if obj and obj.id and obj.inventory_use_slot in 
            (WieldLocation.UPPER_BODY, WieldLocation.LOWER_BODY, WieldLocation.HEAD)
        ]
    ...
    def all(self, only_objs=False):
        ...
        slots = self.slots
        lst = [
            (slots[WieldLocation.LEFT_HAND], WieldLocation.LEFT_HAND),
            (slots[WieldLocation.RIGHT_HAND], WieldLocation.RIGHT_HAND),
            (slots[WieldLocation.TWO_HANDS], WieldLocation.TWO_HANDS),
            (slots[WieldLocation.UPPER_BODY], WieldLocation.UPPER_BODY),
            (slots[WieldLocation.LOWER_BODY], WieldLocation.LOWER_BODY),
